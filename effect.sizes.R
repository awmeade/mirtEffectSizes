...
f.effect.sizes <- function(focal.theta.obs, model){
  require("mirt")
  ...
  for(i in 1:ncol(dat)){
    foc.extract<-extract.item(model,i,group=1)
    ref.extract<-extract.item(model,i,group=2)
  ...
  #cohen D and D max
  ETSSD <- f.cohen.d(ETS.ref.obs, ETS.foc.obs) # cohen's D
